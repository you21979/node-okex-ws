...
const gzipDecode = (buffer) => {
    return new Promise(( resolve, reject ) => {
        zlib.inflateRaw(buffer, (err, decoded_binary) => {
            if(err) reject(err)
            else resolve(decoded_binary)
        })
    })
}

const initializeStream = (self) => {
    const ws = new WebSocket(self.url + "?compress=true");
    ws.on('open', () => {
        self.onConnected()
    });
    ws.on('message', (data) => {
        gzipDecode(data).then( binary => {
            const msg = JSON.parse(binary.toString())
            msg.forEach(v => {
